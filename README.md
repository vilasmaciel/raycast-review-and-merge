...
- **Pull Requests Menu Bar** — a macOS menu-bar entry that polls GitHub in the background and shows, in one dropdown, the PRs **awaiting your review** and **your own** open PRs. The icon shows a badge with the number of PRs awaiting your review. Each PR is a one-click row: its icon and tooltip reflect the smart action, clicking runs it (**Approve**, **Merge**, or **Enable Auto-Merge** depending on state), and holding **⌥** opens the PR in the browser instead. Each row's subtitle shows its repository, number, and checks/review status. It refreshes in the background every minute; you can turn that background refresh on or off from Raycast's command settings.
